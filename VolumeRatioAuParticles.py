...
au = df['Au particle?'].fillna('NA').values

for val, color in [('No', 'tab:blue'), ('Yes', 'tab:orange')]:
    m = au == val
    ax.scatter(x[m], ratios[m], s=70, color=color, edgecolor='k', zorder=3,
               label=f'Au particle: {val}')
