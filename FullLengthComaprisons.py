...
def draw(color_by):
    ...
    for xpos, df in enumerate(data_center):
        ...
        if color_by == 'None':
            ...
            sub = df[["total", color_by]].dropna(subset=["total"])
            ...
            for val in flagvals.unique():
                ...
                ax.scatter(xpos + jitter, rows["total"].values,
                           s=70, alpha=0.8, edgecolor='k', c=color, zorder=3,
                           label=label if label not in seen else None)
                seen.add(label)
                sub_value = rows["total"].values
    ...
    title = "Total length by data sample"
    if color_by != 'None':
